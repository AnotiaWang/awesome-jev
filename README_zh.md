...
- [jev-ego](https://github.com/romaluev/jev-ego) - 同一套 Jev Ultrafast 策略，跑在 [ego lite](https://lite.ego.app/) 上：一次 TypeSafe 请求选出操作和编号元素，TypeScript CLI，`observe`/`act`/`suggest`/`step` 守护进程。不用 Chrome 或 Playwright
...
- [jev-ego](https://github.com/romaluev/jev-ego) - 同上项目；CLI stepper + skill，Agent 可在已打开的 ego lite TaskSpace 上 `observe`/`act`/`suggest`
