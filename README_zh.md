...
- [jev-style](https://github.com/lawrence3699/jev-style) - 非官方 0.8B 决策模型（Qwen3.5 微调），`pip install jev-style` 即可在本地运行（MLX、PyTorch 或 llama.cpp），提供兼容 `/v1/systemone` 的服务：一次前向回答 Choice、Score、Noul，并附带 MCP 服务和 Claude Code 守门钩子。非官方，不是 TypeSafe 的模型。
