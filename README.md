...
- [jev-ego](https://github.com/romaluev/jev-ego) - Same Jev Ultrafast policy on [ego lite](https://lite.ego.app/): one TypeSafe request for operation + indexed element, TypeScript CLI, `observe`/`act`/`suggest`/`step` daemon. No Chrome or Playwright
...
- [jev-ego](https://github.com/romaluev/jev-ego) - Same project as above; CLI stepper + skill so an agent can `observe`/`act`/`suggest` on a live ego lite TaskSpace
